function Update-NVidiaDriver {
	param (
		[Parameter(Mandatory=$false, ParameterSetName="Clean")][switch]$Clean
	)
  Write-Host "Updating NVidia Driver's to the latest version:" -ForegroundColor "Green";
	# Checking if Windows version is at least 10.0 (Windows 10)
	if ([System.Version][Environment]::OSVersion.Version.ToString() -lt [System.Version]"10.0") {
		Write-Host "Your Windows version is unsupported. Upgrade Windows 10 or higher" -ForegroundColor "Red";
		exit;
	}
	# Checking processor compatibility
	if (-not [Environment]::Is64BitOperatingSystem) {
    Write-Host "32-bit processors on x86 architecture is nsupported. Upgrade a 64-bit processor on x64 architecture" -ForegroundColor "Red";
		exit;
	}
	...
	Write-Host "Current version: $CurrentDriverVersion" -ForegroundColor "Green";
