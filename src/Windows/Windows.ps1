function Set-WindowsExplorer-ShowHiddenFiles {
  Write-Host "Configuring Windows File Explorer to show hidden files, folders, and drives:" -ForegroundColor "Green";
  $RegPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced";
  Set-ItemProperty -Path $RegPath -Name "Hidden" -Value 1;
}

function Set-WindowsExplorer-ShowFileExtensions {
  Write-Host "Configuring Windows File Explorer to show file extensions for known file types:" -ForegroundColor "Green";
  $RegPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced";
  Set-ItemProperty -Path $RegPath -Name "HideFileExt" -Value 0;
}

function Set-WindowsExplorer-ShowOperatingSystemFiles {
  Write-Host "Configuring Windows File Explorer to show protected operating system files:" -ForegroundColor "Green";
  $RegPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced";
...
function Set-Power-Configuration {
  ...
  # Turn Battery Saver on automatically at 25%
  powercfg /SETDCVALUEINDEX SCHEME_CURRENT SUB_ENERGYSAVER ESBATTTHRESHOLD 25
...
Set-WindowsExplorer-ShowHiddenFiles;
Set-WindowsExplorer-ShowFileExtensions;
Set-WindowsExplorer-ShowOperatingSystemFiles;
